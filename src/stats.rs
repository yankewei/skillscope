...
fn print_skill_stats(db: &Database, since: Option<String>, json: bool) -> Result<()> {
    ...
    println!(
        "{:<32} {:>8} {:>9} {:>9} {:<25} {:<25}",
        "skill", "total", "explicit", "implicit", "first_seen", "last_seen"
    );
    for stat in stats {
        println!(
            "{:<32} {:>8} {:>9} {:>9} {:<25} {:<25}",
...
fn print_invocation_type_stats(db: &Database, since: Option<String>, json: bool) -> Result<()> {
    ...
    println!(
        "{:<18} {:>8} {:<25} {:<25}",
        "invocation_type", "total", "first_seen", "last_seen"
    );
    for stat in stats {
        println!(
            "{:<18} {:>8} {:<25} {:<25}",
...
fn invocation_type_stats(db: &Database, since: Option<&str>) -> Result<Vec<InvocationTypeStat>> {
    let sql = if since.is_some() {
        r#"
        SELECT invocation_type, COUNT(*) AS total_count, MIN(timestamp), MAX(timestamp)
        FROM skill_invocations
        WHERE timestamp >= ?1
        GROUP BY invocation_type
        ORDER BY total_count DESC, invocation_type ASC
        "#
    } else {
        r#"
        SELECT invocation_type, COUNT(*) AS total_count, MIN(timestamp), MAX(timestamp)
...
fn map_skill_stat(row: &rusqlite::Row<'_>) -> rusqlite::Result<SkillStat> {
    Ok(SkillStat {
        ...
        total: row.get::<_, i64>(1)? as u64,
        explicit: row.get::<_, i64>(2)? as u64,
        implicit: row.get::<_, i64>(3)? as u64,
